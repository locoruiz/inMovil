...
function guardar(){
	...
	try{
		...
		$sql = "insert into Inmuebles values (0, 2, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
function modificar(){
	...
	try{
		...
		$idDepar = 0;
		if(!isset($_POST["idDepar"]) || $_POST["idDepar"] <= 0)
		...
		$id = $con->EjecutarPrepared($sql, array($descripcion, $detalle, $precioVenta, $precioAlquiler, $precioAnticretico,
												 ...
												 $cuartos, $banos, $pisos, $direccion, $latitud,
												 $longitud, $zona, $barrio, $provincia, $fechaIni,
												 $fechaFin, $nombre, $telefono, $idDepar));

		$sql = "delete from Fotos where tipo = 1 and id_rel = ".$con->validar($idDepar);
		$con->EjecutarSQL($sql);
		...
		for($i = 1; $i <= $img; $i++){
			...
			if(file_exists($target_dir.$nombre)){
				...
				$con->EjecutarPrepared($sql, array(1, $idDepar, $i, $nombre));
			}else{
				$indices[$nombre] = $i;
			}
		}
		...
		for($j=0; isset($_FILES["fotos"]) && $j < count($_FILES["fotos"]['name']); $j++){
			...
			$indice = time() + $j;
			$nombre = "1_".$idDepar."_".$indice.".".$imageFileType;
			...
			if (move_uploaded_file($_FILES["fotos"]["tmp_name"][$j], $target_file)) {
				...
				$i++;
				$sql = "INSERT INTO Fotos VALUES (1,".$idDepar.",$indice,'".$nombre."')";
				$con->EjecutarSQL($sql);
			} else {
				// Hubo un error
				$uploadOk = 0;
		...
		if ($uploadOk == 0) {
			...
			if($i > 0)
				...
				$resultado["mensaje"] = "Modificado correctamente";
		}
		$resultado["success"] = 1;
		echo json_encode($resultado);
	}catch(Exception $e){
